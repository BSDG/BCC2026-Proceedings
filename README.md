# Boise Code Camp 2026 Proceedings

Welcome to the official session proceedings from Boise Code Camp 2026. An open check box means we are still working with the speakers to collect their slides, code, etc. into their folder. If the session you are looking for isn't available yet please check back. Thanks!

- [ ] [AI & LLM: Existential Threat, or Shiny New Tool? - Panel](sessions/AI-LLM_ExistentialThreat/README.md)
- [ ] [Back to Basics: Essential JavaScript Foundations for Modern Framework Development - Nerando Johnson](sessions/NerandoJohnson-EssentialJavaScriptFoundations/README.md)
- [ ] [Beyond the Prompt: Building Digital Coworkers with Agentic Workflows - Travis Dacolias](sessions/TravisDacolias-AgenticCoworkers/README.md)
- [ ] [Browser customization: beyond the basics - Jacob Winters](sessions/JacobWinters-BrowserCustomization/README.md)
- [ ] [Building Custom MCP Servers in .NET - Benjamin Michaelis](sessions/BenjaminMichaelis-CustomDotNetMcp/README.md)
- [x] [C Without a Build System or: How I Killed the Linker - Samuel Lieberman](sessions/SamuelLieberman-CKilledLinker/README.md)
- [ ] [Code Your Own Arcade Adventure - Duane Erickson](sessions/DuaneErickson-ArcadeAdventure/README.md)
- [ ] [From Cloud Native to Multi-Cloud Native: Write Once, Deploy Anywhere - Sandeep Pal](sessions/SandeepPal-Multi-CloudNative/README.md)
- [x] [From OpenClaw to Hermes: Running Local Autonomous Agents in 2026 - Charles "Chuck" Anderson](sessions/CharlesAnderson-OpenclawHermesAgents/README.md)
- [ ] [From Scripted to Smart: How to Systematically Humanize Your AI Sales Agent - Lior Mechlovich](sessions/LiorMechlovich-SmartHumanizedAgents/README.md)
- [ ] [Getting and keeping a developer job in this market from the perspective of a hiring manager - Jeremy Perez](sessions/JeremyPerez-GettingKeepingDevJobs/README.md)
- [ ] [How Not to Vibe Code Yourself Into a Lawsuit - Doug Applegate](sessions/DougApplegate-VibeCodeLegal/README.md)
- [ ] [How To Beat Burnout Without Pressing Pause - Nate Hebbert](sessions/NateHebbert-BeatBurnout/README.md)
- [x] [Improv Skills for Hackers and Pentesters - Ross Merritt, Jim McKeeth](sessions/RossMerritt-ImprovSkillsHackers/README.md)
- [ ] [Introduction to Kotlin and Jetpack Compose - Doug Reece](sessions/DougReece-IntroductionKotlinJetpack/README.md)
- [ ] [It's Dangerous to Go Alone - Aaron Bynum](sessions/AaronBynum-ItsDangerous/README.md)
- [ ] [Make Your Serverless Functions Do More with Durable Executions - Erik Hanchett](sessions/ErikHanchett-ServerlessDurableExec/README.md)
- [ ] [Mastering the Agentic Coding Workflow - Mark Michaelis, Benjamin Michaelis](sessions/AgenticWorkflow-Michaelis/README.md)
- [ ] [pgFirstAid - Justin Frye](sessions/JustinFrye-pgFirstAid/README.md)
- [ ] [Programmatic 3D Modeling with OpenSCAD - Jeremy Reeder](sessions/JeremyReeder-OpenSCAD3dModeling/README.md)
- [ ] [Robots Don't Need Sprint Boards - Sean Timm](sessions/SeanTimm-RobotsDontSprint/README.md)
- [ ] [Scratch Jr: Code, Create & Play - Duane Erickson](sessions/DuaneErickson-ScratchJrCode/README.md)
- [ ] [Stinky Code - A Review of Common Code Smells - Marlin Roberts](sessions/MarlinRoberts-StinkyCodeReview/README.md)
- [ ] [The Burnout Paradox - Nate Hebbert](sessions/NateHebbert-BurnoutParadox/README.md)
- [ ] [The Engineer's Guide to Socialization - Nerando Johnson](sessions/NerandoJohnson-EngineersGuideSocialization/README.md)
- [ ] [The Future of Software Development: The Path Forward in a Changing Industry - Mark Michaelis, Benjamin Michaelis](sessions/FutureSoftwareDevelopment-Michaelis/README.md)
- [ ] [The Importance of Data in the Era of AI: Why Data Makes AI Real - Srinivasa Rao M](sessions/SrinivasaRaoM-DataInAiEra/README.md)
- [ ] [The Unkillable Scraper: Data Collection That Doesn't Quit - Aaron Goin](sessions/AaronGoin-UnkillableDataScraper/README.md)
- [ ] [Thinking like a SDET -- Testing Software in the Era of AI Development - Jeanne Allen](sessions/JeanneAllen-ThinkingLikeSdet/README.md)
- [ ] [Using Spec-Driven Development for Production Workflows - Erik Hanchett](sessions/ErikHanchett-UsingSpecDrivenDev/README.md)
- [x] [Who Owns the Code? - Richard Hundhausen, Bradlee Frazer](sessions/RichardHundhausen-BradleeFrazer-WhoOwnsCode/README.md)

## License

Each contributor is free to choose their own license for their contributions. So check their README or for a LICENSE file in their folder. Unless otherwise specified, all content is copyright by the creator or original copyright holder and licensed under the MIT License.

[Boise Code Camp](https://boisecodecamp.com) is operated by [Boise Software Developers Group, Inc.](https://bsdg.dev), a state and federal 501(c)3 non-profit organization with the mission of supporting and nurturing technical communities in and around Boise, ID.
